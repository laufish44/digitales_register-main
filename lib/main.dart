...
import 'package:dr/background/windows_background.dart';
...
Future<void> main(List<String> args) async {
  if (Platform.isWindows && args.contains(backgroundModeFlag)) {
    // Started by the autostart entry: no UI, just poll and notify.
    await runWindowsBackgroundService();
    return;
  }
  ...
  if (Platform.isWindows) {
    // Lets the background service know it can stay quiet while we are open.
    unawaited(announceAppIsRunning());
  }
  try {
  ...
  Widget build(BuildContext context) {
    return ReduxProvider(
      ...
      child: Listener(
        ...
        // The chosen preset is read from the store here so that changing it
        // rebuilds DynamicTheme with a new `data` callback, which makes it
        // recompute the theme (see DynamicThemeState.didUpdateWidget).
        child: StoreConnection<AppState, AppActions, String>(
          connect: (state) => state.settingsState.themePreset,
          builder: (context, presetId, actions) => DynamicTheme(
            data: (brightness, overridePlatform) {
              TargetPlatform? platform;
              if (overridePlatform && Platform.isAndroid) {
                platform = TargetPlatform.iOS;
              }
              return themePresetById(presetId).build(brightness, platform);
            },
