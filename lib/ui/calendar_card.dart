...
/// How an absence is shown in the calendar.
class AbsenceStyle {
  const AbsenceStyle({
    required this.color,
    required this.icon,
...
/// The colour, icon and wording for a lesson's absence state.
///
/// Returns null when there is nothing to show.
AbsenceStyle? absenceStyle(BuildContext context, AbsenceMark mark) {
  switch (mark) {
    case AbsenceMark.none:
      return null;
    case AbsenceMark.announced:
      return const AbsenceStyle(
        color: Colors.orange,
        icon: Icons.event_busy,
        label: "Vorentschuldigt",
      );
    case AbsenceMark.justified:
      return const AbsenceStyle(
        color: Colors.green,
        icon: Icons.check_circle_outline,
        label: "Absenz, entschuldigt",
      );
    case AbsenceMark.notJustified:
      return AbsenceStyle(
        color: Theme.of(context).colorScheme.error,
        icon: Icons.error_outline,
        label: "Absenz, nicht entschuldigt",
      );
  }
}
...
class CalendarCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final absence = absenceStyle(context, absenceMark);
    return Card(
      shape: RoundedRectangleBorder(
        ...
        side: selected
            ...
            : absence != null
                ? BorderSide(color: absence.color, width: 1.5)
                : BorderSide.none,
      ),
      color: absence != null
          ? Color.alphaBlend(
              absence.color.withValues(alpha: 0.08),
          ...
          children: [
            ...
            if (absence != null)
              Padding(
                ...
                child: Row(
                  children: [
                    Icon(absence.icon, size: 16, color: absence.color),
