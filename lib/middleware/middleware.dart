...
///
/// Silent when nothing is configured, nothing is newer, or the server cannot
/// be reached — an update check must never get in the way of using the app.
...
Future<String> _getAttachmentDownloadDirectory() async {
  if (isDesktop()) {
    // getExternalStorageDirectories is Android only and throws everywhere else,
    // so on desktop we ask for the real "Downloads" folder instead. This also
    // matches what the dialog in [askShouldOverwriteFile] tells the user.
    try {
      final downloads = await getDownloadsDirectory();
      if (downloads != null) return downloads.path;
    } catch (e) {
      log("failed to get the downloads directory", error: e);
    }
  } else {
    try {
...
/// The location an attachment named [fileName] is stored at.
Future<File> _attachmentFile(String fileName) async {
  return File(
    joinPath(await _getAttachmentDownloadDirectory(), sanitizeFileName(fileName)),
  );
}
...
Future<bool> downloadFile(
  ...
  final saveFile = await _attachmentFile(fileName);
  var success = true;
...
/// Sends a request to the register's API.
///
/// [wrapper] itself is `@visibleForTesting`, so code outside the middleware
/// goes through here instead of reaching into it.
Future<dynamic> sendApiRequest(
  String url, {
  Map<String, Object?> args = const <String, Object?>{},
}) =>
    wrapper.send(url, args: args);

Future<bool> canOpenFile(String fileName) async {
  return (await _attachmentFile(fileName)).existsSync();
}

/// Opens a downloaded attachment with the default application for its type.
///
/// Shows a snack bar if that does not work, rather than letting the failure
/// bubble up into the generic error screen.
Future<void> openFile(String fileName) async {
  final result = await openFileWithDefaultApp(
...
///
/// Without this a file that cannot be downloaded would be retried on every
/// single refresh.
final _prefetchAttempted = <String>{};

@visibleForTesting
void resetPrefetchState() => _prefetchAttempted.clear();

/// Downloads attachments in the background so they can be opened offline.
///
/// Only runs when the user turned it on. Failures are ignored on purpose: this
/// is a convenience, and the attachment is still downloaded on demand when it
/// is tapped.
Future<void> prefetchAttachments(
    MiddlewareApi<AppState, AppStateBuilder, AppActions> api) async {
  if (!api.state.settingsState.prefetchAttachments) return;
  if (api.state.noInternet || api.state.isDemo) return;
  ...
  Future<void> fetch(
    ...
    String url,
    Map<String, dynamic> parameters,
    Future<void> Function(bool success) onDone,
  ) async {
    if (!_prefetchAttempted.add(uniqueName)) return;
    if (await canOpenFile(uniqueName)) {
      await onDone(true);
      return;
    }
    final success = await downloadFile(url, uniqueName, parameters);
  ...
  for (final message in api.state.messagesState.messages) {
    for (final attachment in message.attachments) {
      await fetch(
        ...
        (success) => api.actions.messagesActions.fileAvailable(
          attachment.rebuild((b) => b..fileAvailable = success),
        ),
      );
    }
  }

  for (final day in api.state.calendarState.days.values) {
    for (final hour in day.hours) {
      for (final content in hour.lessonContents) {
        for (final submission in content.submissions) {
          await fetch(
            ...
            (success) => api.actions.calendarActions.fileAvailable(
  ...
  for (final day in api.state.dashboardState.allDays ?? const <Day>[]) {
    for (final homework in day.homework) {
      for (final submission
          in homework.gradeGroupSubmissions ?? const <GradeGroupSubmission>[]) {
        await fetch(
          submission.uniqueName,
          ...
          (success) => api.actions.dashboardActions.attachmentReady(
            submission.rebuild((b) => b..fileAvailable = success),
          ),
        );
      }
