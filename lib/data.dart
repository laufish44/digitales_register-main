...
abstract class AbsenceGroup
  ...
  /// The untouched JSON the server sent for this group.
  ///
  /// `api/student/dashboard/absence_reason` expects the whole group back with
  /// only the justification fields filled in, including keys this app does not
  /// model (ids, self declaration details). Keeping the original means we can
  /// hand it back verbatim instead of guessing what the server needs.
  String? get raw;
...
abstract class FutureAbsence
  ...
  /// The untouched JSON the server sent, see [AbsenceGroup.raw].
  /// `remove_absence_future` needs the original object back.
  String? get raw;
...
/// A ready-made justification reason the school offers ("Selbsterklärung"),
/// e.g. "Krankheit" — this is what the website calls "krank melden".
abstract class SelfDeclaration
  ...
  /// The wording that ends up as the justification text.
  String get text;

  /// Whether the student additionally has to type something.
  bool get inputMandatory;

  /// Explains what that something should be.
  String get inputExplain;

  static void _initializeBuilder(SelfDeclarationBuilder b) => b
    ..title = ""
    ..text = ""
...
abstract class Message implements Built<Message, MessageBuilder> {
  ...
  /// The recipient. Zero for messages the user sent themselves.
  int get toUserId;

  /// The sender, needed to prefill the recipients when replying.
  int? get fromUserId;

  /// The sender expects a written answer, and this is it (null until given).
  bool get responseRequired;
  String? get response;

  /// The sender expects the message to be signed, and this is the signature.
  bool get signatureRequired;
  String? get responseSignature;

  /// Set when a parent has to sign instead of the student, in which case the
  /// student's own view is not waiting for anything.
  bool get needsParentSignature;

  /// Whether the message still needs the user's attention.
  ///
  /// Mirrors `setUnreadIdentificator` in the web app: opening a message is not
  /// enough while an expected answer or signature is still missing, and
  /// messages the user sent themselves are never flagged. The app used to treat
  /// "has a timeRead" as read, which is why messages that require a response
  /// kept coming back as new after a restart - the server rightly never
  /// considered them dealt with.
  bool get isNew {
    if (toUserId <= 0) return false;
    final answered = needsParentSignature || !responseRequired || response != null;
    final signed =
        needsParentSignature || !signatureRequired || responseSignature != null;
    return !(timeRead != null && answered && signed);
  }

  /// True when the message was opened but is still waiting for the user to do
  /// something, so the UI can explain why it is still marked.
  bool get needsAction => isNew && timeRead != null;
