...
final messagesReducerBuilder = NestedReducerBuilder<AppState, AppStateBuilder,
  ...
  ..add(MessagesActionsNames.markedAsRead, _markedAsRead);
...
void _markAsRead(
  ...
  // Only a guess until the server confirms with its own timestamp; see
  // MessagesActionsNames.markedAsRead.
  builder.messages[index] = state.messages[index].rebuild(
    (b) => b..timeRead = now,
  );
}

/// Applies the timestamp the server reported, so the local state matches what
/// the next `getMyMessages` will return.
void _markedAsRead(MessagesState state, Action<MarkedAsReadPayload> action,
    MessagesStateBuilder builder) {
  final index = state.messages.indexWhere((m) => m.id == action.payload.id);
  if (index == -1) return;
  builder.messages[index] = state.messages[index].rebuild(
    (b) => b..timeRead = action.payload.timeRead,
  );
}
...
Message _parseMessage(Map json, MessagesState state) {
  final message = MessageBuilder()
    ...
    ..id = getInt(json["id"])
    ..toUserId = getInt(json["toUserId"]) ?? 0
    ..fromUserId = getInt(json["fromUserId"])
    ..responseRequired = getBool(json["responseRequired"]) ?? false
    ..response = getString(json["response"])
    ..signatureRequired = getBool(json["signatureRequired"]) ?? false
    ..responseSignature = getString(json["responseSignature"])
    ..needsParentSignature = getBool(json["needsParentSignature"]) ?? false;
